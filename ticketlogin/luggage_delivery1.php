    ...
    <table width="100%" class="arrow_box_content top_arrow">
      ...
      <tr>
        ...
        <td width="80" align="center" class="label_txt">&nbsp;</td>
						...
						<h6 class="inline_code">Luggage Delivery : Vehicle Wise -
						<?php  
						$vecstr = '';
						...
						else
						{
							...
							echo $vehname[0];
							$vecstr =' and luggage_vehicle = "'.$_POST['vehicle'].'" and luggage_to = '.$_SESSION['route'].'';
						}
								
								?> [ <?php echo date('d-M-Y',strtotime($frmdate)).' To '.date('d-M-Y',strtotime($todate));?> ] <span class="label_txt" style="text-transform:uppercase;">
						
						<?php if($_REQUEST['msg']=='inwrd') echo "<br><br><font color=red>Updated Successfully.</font>";
...
<br>
                    ...
                    <div align="right"><div class="btn_40_blue ucase">
								<a href="javascript:deleteAll();"><span class="icon finished_work_sl"></span><span>Click to Luggage Inward</span></a>
							</div></div>
					<div class="widget_content">
	...
	if($_POST['Submit'] || (isset($_REQUEST['frmdate']) && isset($_REQUEST['todate'])))
{
	...
	?>
						...
						<thead>
						<tr>
						  <th width="4%"><span class="yellow_pro">INWARD</span>	<input type="checkbox"  name="chkAll"  id="chkAll" onclick="checkAll();"></th>
							<th width="4%">
								 Sl No
							</th>
							<th width="7%">Date</th>
                   ...
                   echo "select * from power_ticket where create_date>='".$frmdate."' and create_date<='".$todate."' $vecstr";
					 $luggage=mysqli_query($objConn,"select * from power_ticket where create_date>='".$frmdate."' and create_date<='".$todate."' $vecstr") or die("Vehicle error:".mysql_error());
				   ...
				   ?>
						<tr <?php if($css_set==0) echo "class=gradeX"; else  echo "class=gradeC"; ?>>
						  <td align="center">
                           <?php if($luggage_row['luggage_status']=='Dispatched') 
						{
						...
						?>
							...
							<td align="center"><?php echo $tot_amt=$luggage_row['luggage_fees']+$luggage_row['luggage_charges']+$luggage_row['luggage_doorcharges']+$luggage_row['unloading_charges']?></td>
...
<img src="images/edit.png"/></a></span>
                            <span><a href="luggage_delivery.php?news=del&amp;event_id=<?php echo $luggage_row['id'];?>" onclick="return confirmSubmit();" title="delete"><img src="images/delete.png"/></a></span><span><a href="luggage_edit.php?news=del&amp;event_id=<?php echo $luggage_row['id'];?>" onclick="return confirmSubmit();" title="delete"></a></span>
							</td>
						</tr>
                    ...
                    $luggage=mysqli_query($objConn,"select * from power_ticket where luggage_to = '".$_SESSION['route']."' and create_date>='".$frmdate."' and create_date<='".$todate."' ") or die("Vehicle error:".mysql_error());
					$luggage_num=mysqli_num_rows($luggage);
					if($luggage_num==0)
					{
                    ?>
				   ...
				   ?>
						<tr <?php if($css_set==0) echo "class=gradeX"; else  echo "class=gradeC"; ?>>
						  <td align="center"> <?php if($luggage_row['luggage_status']=='Dispatched') 
						{
							?><input type="checkbox" name="del[]"  id="del[]" value="<?php echo $luggage_row['id']; ?>"  > 
							<?php
						}else
...
<img src="images/edit.png"/></a></span>
						...
						if($luggage_row['luggage_paymethod']=='Credit')
						 $credit_amt +=$tot_amt;
						$i++;} } if($luggage_num!=0)
						{?><tr><td colspan="18" align="right"  bgcolor="#F9C6F1"><strong>Paid Amount : 
                        <?php 
