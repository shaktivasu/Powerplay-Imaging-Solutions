	...
	if($_POST['Submit'] || (isset($_REQUEST['frmdate']) && isset($_REQUEST['todate'])))
{
	...
	?>
					...
					if($_POST['lrno']!='')
					{
					 ...
					 $luggage=mysqli_query($objConn,"select * from power_ticket where $vecstr and (luggage_status = 'New Arrival' || luggage_status = 'Dispatched')   order by id asc") or die("Vehicle error:".mysql_error());
					$luggage_num=mysqli_num_rows($luggage);
					}
					else
					{
					$luggage=mysqli_query($objConn,"select * from power_ticket where create_date>='".$frmdate."' and create_date<='".$todate."' $vecstr   and (luggage_status = 'New Arrival' || luggage_status = 'Dispatched') order by id asc") or die("Vehicle error:".mysql_error());
					$luggage_num=mysqli_num_rows($luggage);	
					}
					if($luggage_num==0)
					{
...
<img src="images/edit.png"/></a></span>
                    ...
                    $luggage=mysqli_query($objConn,"select * from power_ticket where  luggage_status = 'New Arrival' and luggage_to = '".$_SESSION['route']."' and create_date>='".$frmdate."' and create_date<='".$todate."' ") or die("Vehicle error:".mysql_error());
					$luggage_num=mysqli_num_rows($luggage);
					if($luggage_num==0)
					{
                    ?>
				   ...
				   ?>
						...
						//}?>
                            ...
                            <?php /*<?php if($luggage_row['luggage_status']=='Inward') {
								?>
							<span>
