...
function val()
{
	if(document.frm.route_agent.value==0)
	{
		alert("Select Any Service Center");
		return false;
	}
}
...
<div id="container">
    ...
    <table width="100%" border="1" cellspacing="0" cellpadding="0" style="border-collapse:collapse;" class="arrow_box_content top_arrow">
      ...
      <tr>
        <td height="26" align="right" class="label_txt">L R NO :</td>
        <td align="center" class="label_txt" style="text-transform:uppercase;"><input type="text"  name="lrno" id="lrno" value="<?php echo $_POST['lrno']?>"></td>
        <td align="right" class="label_txt">&nbsp;</td>
      ...
      <tr>
        <td height="26" align="right" class="label_txt">Select Any SRC :</td>
        <td align="center" class="label_txt" style="text-transform:uppercase;"><select name="route_agent" class="limiterBox">
          <option value="0">Select Any</option>
          <?php 
				  $routewise = "select * from power_route where id in (select luggage_to from power_luggage) order by route";
				  ...
				  $routewise2 = mysqli_num_rows($routewise1);
					...
					{
											if($_POST['route_agent']==$routewise3['id'])
											echo "<option value=".$routewise3['id']." selected>".$routewise3['route']."</option>";
											else
											echo "<option value=".$routewise3['id'].">".$routewise3['route']."</option>";
										}
      ...
      <tr>
       <td colspan="5"></td>
				  ...
				  //$routewise = "select * from power_route where id in (select luggage_to from power_luggage where create_date>='".$frmdate."' and create_date<='".$todate."' $vecstr".")";
				   if($_POST['lrno']!='')
				   {
					...
					//Maximum of News & Events
                     ...
                     $luggage=mysqli_query($objConn,"select * from power_luggage where  luggage_lrno='".$_POST['lrno']."'") or die("Vehicle error:".mysql_error());
				  ...
				  <?php  }
				   ...
				   {
					   $routewise = "select * from power_route where id = ".$_POST['route_agent'];
				  $routewise1 = mysqli_query($objConn,$routewise);
				   $routewise2 = mysqli_num_rows($routewise1);
				   $routewise3 = mysqli_fetch_array($routewise1);
				   if($_POST['route_agent']!=0)
				   {
				 // if($routewise2>0)
				 // {
					//while($routewise3 = mysqli_fetch_array($routewise1))
					//{
				  ?>
						<h6 class="inline_code"><?php echo $routewise3['route'];
						...
						<tbody>
                          <?php
					//Maximum of News & Events
					// echo "select * from power_luggage where  luggage_from = ".$_POST['route_agent']." and luggage_status = 'New Arrival' and create_date between  '".$frmdate."' and '".$todate."'";
                    $luggage=mysqli_query($objConn,"select * from power_luggage where  luggage_from = ".$_POST['route_agent']." and luggage_status = 'New Arrival' and create_date between  '".$frmdate."' and '".$todate."'") or die("Vehicle error:".mysql_error());
					$luggage_num=mysqli_num_rows($luggage);
					if($luggage_num==0)
					{
                    ?>
...
<img src="images/edit.png"/></a></span> <span><a href="luggage.php?news=del&amp;event_id=<?php echo $luggage_row['id'];?>" onclick="return confirmSubmit();" title="delete"><img src="images/delete.png"/></a></span>
							<?php /*?><span>
                           <a href="luggage_edit.php?id=<?php echo $luggage_row['id']; ?>" title="Edit"><img src="images/edit.png"/></a></span>
