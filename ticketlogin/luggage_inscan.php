...
if($_REQUEST['lrno']!="")
{
 ...
 $lrno=$_REQUEST['lrno'];
  $chk = "select * from power_ticket where luggage_lrno = '".$lrno."' and (luggage_status = 'New Arrival' or luggage_status = 'Dispatched') and luggage_lrno in (select luggage_lrno from power_outscan where luggage_status = 'Cleared')";
 ...
 {	 
		  ...
		  $up = "update power_ticket set luggage_status = 'Inward' where luggage_lrno = '".$lrno."'";
		 $up1 = mysqli_query($objConn,$up);
		 header("location:luggage_inscan.php");
 } 
}
  ...
  barcodeInput.addEventListener('input', function() {
    ...
    <?php }?>
      ...
      <tr>
		...
		<input type="hidden" name="route_to" id="route_to" value="<?php echo $_SESSION['route']?>">
		<input type="text" name="lrno" id="lrno" placeholder="Scan barcode here"></td>
        <td align="right" class="label_txt">&nbsp;</td>
        <td align="center" class="label_txt"></td>
        <td align="center" class="label_txt">&nbsp;</td>
      </tr>
      ...
      <tr>
        ...
        <td  align="center" class="label_txt">&nbsp;</td>
					...
					{?>
					 ...
					 $luggage_inscan=mysqli_query($objConn,"select * from power_outscan where DATE(create_date) = CURDATE() and  luggage_status = 'Shortage' and veh_id = ".$_SESSION['vehicle']." and route_to =  ".$_SESSION['route']." order by create_date desc") or die("Vehicle error:".mysql_error());
				   ...
				   ?>
						...
						<tr>
							...
							<th>Booking Date</th><th width="7%">Inward Date</th>
                            <th >LR No.</th>
                            <th>Sender Name &amp; Phone No.</th>
                            <th>Reciever Name &amp; Phone No.</th>
							<th>
					 ...
					 $luggage_inscan=mysqli_query($objConn,"select * from power_outscan where DATE(create_date) = CURDATE() and luggage_status = 'Excess'  and veh_id = ".$_SESSION['vehicle']."  order by create_date desc") or die("Vehicle error:".mysql_error());
					 $luggage_num=mysqli_num_rows($luggage_inscan);
				   ...
				   ?>
						...
						<?php
						 ...
						 $luggage_inscan=mysqli_query($objConn,"select * from power_outscan where DATE(create_date) = CURDATE()  and veh_id = ".$_SESSION['vehicle']." and luggage_status='cleared'   order by route_to") or die("Vehicle error:".mysql_error());
					 $luggage_num=mysqli_num_rows($luggage_inscan);
