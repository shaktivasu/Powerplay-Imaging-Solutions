...
if(isset($_POST['invoice_confirm']))
{
	...
	$max3 = 'KMP'.$max2;
	//KMP1220
	$dup = "select * from power_agent_invoices where invoice_from = '".$_REQUEST['frmdate']."' and invoice_to='".$_REQUEST['todate']."' and credit_id =".$_REQUEST['route_to'];
	$dup1 = mysqli_query($objConn,$dup);
	$dup2 = mysqli_num_rows($dup1);
	if($dup2==0)
	{
	 $ins = "insert into power_agent_invoices (invoice_id,credit_id,invoice_date,invoice_from,invoice_to,invoice_amount,invoice_qty) values ('".$max3."','".$_REQUEST['route_to']."','".date('Y-m-d')."','".$_REQUEST['frmdate']."','".$_REQUEST['todate']."','".$_REQUEST['invoice_amount']."',".$_REQUEST['invoice_qty'].")";
       ...
       <td colspan="5"></td>
                  ...
                  <?php 
				   //$routewise = "select * from power_route where id in (select luggage_to from power_luggage where create_date>='".$frmdate."' and create_date<='".$todate."' $vecstr".")";
                    ...
                    $luggage=mysqli_query($objConn,"select * from power_agent_invoices where  invoice_date between  '".$frmdate."' and '".$todate."'") or die("Vehicle error:".mysql_error());
					$luggage_num=mysqli_num_rows($luggage);
					if($luggage_num!=0)
					{
						$heading_tag = mysqli_fetch_array(mysqli_query($objConn,"select * from power_route where id = ".$_POST['route_to']));
							 $heading_tag1 = $heading_tag['route'];
				   ...
				   ?>
						<tr <?php if($css_set==0) echo "class=gradeX"; else  echo "class=gradeC"; ?>>
							...
							<td align="center">
							<?php 
							$routefrom = mysqli_fetch_row(mysqli_query($objConn,"select route from power_route where id = ".$luggage_row['route_id']));
							$routefrom1 = $routefrom[0];
							echo strtoupper($routefrom1);
							?></td>
							...
							<td align="center"><?php echo date('d-M-Y',strtotime($luggage_row['invoice_to']));?></td>
							<td align="center"><?php echo $luggage_row['closing_balance']?></td>
                            <td align="center"><a href="reports_print_agent.php?id=<?php echo $luggage_row['id']?>&frmdate=<?php echo $luggage_row['invoice_from']?>&todate=<?php echo $luggage_row['invoice_to']?>&route_to=<?php echo $luggage_row['route_id']?>&printpreview=yes" target="_blank"><img src="images/printer.png"></a></td>
