...
if($bill_num==0) 
{
	 ...
	 if($_POST['pay_method']=='Credit')
	 {
		 ...
		 if($creditcust2==0)
		 {
			...
			$ins = mysqli_query($objConn,"insert into power_customers (cust_name , cust_phone , cust_address , cust_route , create_date) values ('".$recievername."','".$recieverPhone."','".$recieverAddress."','".$_SESSION['route']."','".date('Y-m-d')."')");
	 ...
	 else
	 {
		 ...
		 if($creditcust2==0)
		 {
			...
			$ins = mysqli_query($objConn,"insert into power_customers (cust_name , cust_phone , cust_address , cust_route , create_date) values ('".$sendername."','".$senderPhone."','".$senderAddress."','".$_SESSION['route']."','".date('Y-m-d')."')");
	   ...
	   $purchs_dtls = "insert into power_ticket set luggage_lrno = '".$_POST['lrno']."',
												 ...
												 unloading_charges = '".$_POST['unluggage_amount']."',
												 ...
												 create_date = '".date('Y-m-d')."',luggage_time = '".date("Y-m-d H:i:s")."'";
	...
	for($i=0; $i<$purchse_count; $i++)
	{
	  ...
	  $particulars_name = mysqli_fetch_array(mysqli_query($objConn,"select particulars from power_particulars where id=".$_POST['prd_name'][$i]));
	   ...
	   if($_POST['pay_method']=='Credit')
	   $unittype = $_POST['credit_item'];
	   else
	   $unittype = $_POST['item_type'][$i];
	  
		echo  $pur_prd_dtls = "insert into power_ticket_prd set luggage_lrno = '".$_POST['lrno']."',
														  ...
														  unit_price = '".$_POST['prd_unitprice'][$i]."',
