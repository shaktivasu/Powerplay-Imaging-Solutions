...
function val()
{
   ...
   if(document.bookingform.credit_code.value==0)
    {
	   document.bookingform.credit_code.focus();
	   alert("Please select any Credit Cleient.");
       return false;
    }
		...
		<div class="grid_container">
		  <div class="grid_12">
		    <div class="widget_wrap">
		      ...
		      <form id="bookingform" name="bookingform" autocomplete="off" method="post"  class="form_container left_label" action="luggage_new_credit.php" onSubmit="return val();"  >
              ...
              <h3>Select Any Credit Client   <select name="credit_code" class="limiterBox" tabindex="2">
                        <option value="0">Select Any</option>
                          <?php 
										$route = mysqli_query($objConn,"select * from power_credit where credit_status=0 order by credit");
										while($route1 = mysqli_fetch_array($route))
										{
										...
										?>
                        </select><!--  <input type="text" name="credit_code" id="credit_code"  class="large">--> &nbsp;  &nbsp; Location To &nbsp;  &nbsp; <select name="route_to" class="limiterBox" tabindex="2">
                        <option value="0">Select Any</option>
                          <?php 
										$route = mysqli_query($objConn,"select * from power_route where route_status=0 and route_nature != 'Booking' order by route");
										while($route1 = mysqli_fetch_array($route))
										{
											if($_SESSION['route']!=$route1['id'])
          ...
          <font color="red" style="font-size:22px">  <input type="hidden" name="manualtype" value="<?php $_REQUEST['type']?>">
           <input type="submit" name="cc_sub" class="not_valid" value="Get Customer" onClick="return val();"></font></h3>
