...
if(isset($_POST['MM_Report']) && $_POST['MM_Report'] == 'Report') {
	$where  =' AND';
	if(isset($_POST['month']) && $_POST['month'] != '') {
		$where .= ' MONTH(order_date)='.$_POST['month'].' AND ';
	}
	if(isset($_POST['category_id']) && $_POST['category_id'] != '') {
		$where .= ' power_products.category_id='.$_POST['category_id'].' AND ';
	}
	if((isset($_POST['from_date']) && $_POST['from_date'] != '') && (isset($_POST['to_date']) && $_POST['to_date'] != '')) {
		$from_date = Date_JS_SQL($_POST['from_date']);
		$to_date = Date_JS_SQL($_POST['to_date']);
		//both date are selected
		$where .= ' order_date BETWEEN "'.$from_date.'" AND "'.$to_date.'" AND ';
	} else {
		if(isset($_POST['from_date']) && $_POST['from_date'] != '') {
			$from_date = Date_JS_SQL($_POST['from_date']);
			$where .= ' order_date >= "'.$from_date.'" AND ';
		} 
		if(isset($_POST['to_date']) && $_POST['to_date'] != '') { 
			$to_date = Date_JS_SQL($_POST['to_date']);
			$where .= ' order_date <= "'.$to_date.'" AND ';
		}
	}
	if(isset($_POST['transaction_mode']) && $_POST['transaction_mode'] != '') {
		$where .= ' power_orders.transaction_mode="'.$_POST['transaction_mode'].'" AND ';
	}
	$where = substr($where, 0, strlen($where)-5);
}

mysql_select_db($database_objConn, $objConn);
$query_product_detail = "SELECT power_orders.order_date, power_orderchild.product_id, power_products.name FROM power_orderchild INNER JOIN (power_products, power_orders) ON (power_orderchild.product_id = power_products.id AND power_orders.order_id = power_orderchild.order_id) ".$where." GROUP BY product_id";
...
if($totalRows_product_detail) {
	do {
		$produt[ucfirst($row_product_detail['name'])] = Get_Product_Sales_Quantity($row_product_detail['product_id'])."--".$row_product_detail['product_id'];
        ...
        <td height="300" align="left" valign="top" bgcolor="#FFFFFF"><table width="95%" border="0" align="center" cellpadding="0" cellspacing="0">
          <tr>
            <td height="25" align="right" valign="top"><table width="100%" border="0" cellspacing="0" cellpadding="0">
                ...
                <tr>
                  <td colspan="2" align="center" valign="middle" class="pagehead">Sales Report Result</td>
                  </tr>
            </table></td>
          ...
          <tr>
            <td align="right" valign="top"><a href="javascript:showreport('1')" class="add_delete">Sort by Product Name</a>&nbsp;&nbsp;&nbsp;<a href="javascript:showreport('2')" class="add_delete">Sort by Sales Quantity</a></td>
          ...
          <tr>
            <td align="left" valign="top"><table width="100%" border="0" cellpadding="3" cellspacing="1" bgcolor="#E3C277">
              <tr>
                ...
                <td width="31%" height="25" align="center" valign="middle" bgcolor="#e1ad45" class="tablehead">Sales  Quantity</td>
              </tr>
              	...
              	<td colspan="2" bgcolor="#F9F2E2">
                	...
                	<table width="100%" border="0" cellpadding="3" cellspacing="1" bgcolor="#E1AD45">
                        ...
                        <tr>
                        	...
                        	<td width="255" align="center" valign="middle" bgcolor="#f9f2e2" class="tabletext"  <?php echo $bg?>>
