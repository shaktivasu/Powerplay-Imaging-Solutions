...
<br>
				  ...
				  { 
				    $vecstr = substr($vecstr,4,strlen($vecstr));
				     $routewise = "select * from power_route where id in (select luggage_to from power_ticket where $vecstr and luggage_status='New Arrival' ".")";
				  }
				  else
				  {
					     $routewise = "select * from power_route where id in (select luggage_to from power_ticket where luggage_status='New Arrival' and create_date>='".$frmdate."' and create_date<='".$todate."' $vecstr".")";
				  }
				  //echo $routewise;
	...
	if($_POST['Submit'])
{
	...
	?>
                    ...
                    //echo "select * from power_ticket where luggage_to = ".$routewise3[0]." and create_date>='".$frmdate."' and create_date<='".$todate."' $vecstr";
					  $luggage=mysqli_query($objConn,"select * from power_ticket where luggage_status='New Arrival' and luggage_to = ".$routewise3[0]." and create_date>='".$frmdate."' and create_date<='".$todate."' $vecstr") or die("Vehicle error:".mysql_error());
				   ...
				   ?>
				    <table class="display">
						<thead>
						<tr>
						  <th width="6%"><span class="yellow_pro">DISPATCH</span> 	<?php if($chk_count==1) {?> <input type="checkbox"  name="chkAll"  id="chkAll" onclick="checkAll();"><?php }?></th>
							<th width="3%">
								 Sl No
							</th>
							<th width="4%">Date</th>
				   ...
				   ?>
						...
						?>
							...
							{
								echo "<font color=red>(".$luggage_row['luggage_ref'].")</font>";
							}
...
<img src="images/edit.png"/></a></span>
					  ...
					  <table class="display">
						...
						<tbody>
                          <?php
                    $luggage=mysqli_query($objConn,"select * from power_ticket where luggage_from = ".$_SESSION['route']." and  luggage_to = ".$routewise3[0]." and create_date>='".date('Y-m-d')."' and create_date<='".date('Y-m-d')."'") or die("Vehicle error:".mysql_error());
					$luggage_num=mysqli_num_rows($luggage);
					if($luggage_num==0)
					{
                    ?>
				   ...
				   ?>
						...
						?>
							...
							if($luggage_row['luggage_status']=='New Arrival')
							{?>
							<a href="#" onclick="return CenterWindow(750,500,50,'print.php?lr=<?php echo $luggage_row['luggage_lrno']; ?>')"><img src="images/print.png"/></a> <?php echo substr($luggage_row['luggage_status'],0,9); }
							else
							echo $luggage_row['luggage_status'];
							?></font>
                            </td>
							<td align="center" class="center"><?php if($luggage_row['luggage_status']=='New Arrival' ) {
								?>
							<span>
                           <a href="#" onclick="return CenterWindow(500,700,50,'luggage_edit.php?id=<?php echo $luggage_row['id']; ?>')"><br>
<img src="images/edit.png"/></a></span>
