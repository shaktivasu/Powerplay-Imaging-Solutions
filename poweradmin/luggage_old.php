...
<br>
					...
					<div class="widget_content">
                  <?php 
				  $routewise = "select * from power_route where id in (select luggage_from from power_luggage where create_date>='".$frmdate."' and create_date<='".$todate."' $vecstr".")";
	...
	if($_POST['Submit'])
{
	...
	?>
                  ...
                  //  echo "select * from power_luggage where create_date>='".$_POST['frmdate']."' and create_date<='".$_POST['todate']."' $vecstr";
					$luggage=mysqli_query($objConn,"select * from power_luggage where ((luggage_from = ".$routewise3[0]." and luggage_paymethod = 'Paid') or (luggage_to = ".$routewise3[0]." and luggage_paymethod = 'To Pay')) and create_date>='".$frmdate."' and create_date<='".$todate."' $vecstr") or die("Vehicle error:".mysql_error());
				   ...
				   ?>
						<tr <?php if($css_set==0) echo "class=gradeX"; else  echo "class=gradeC"; ?>>
							...
							<td align="center" valign="middle" class="badge_style b_pending"><?php echo $luggage_row['luggage_status']?></td>
						...
						{?>
                         <tr>
                        <td colspan="16" align="right"  bgcolor="#F9C6F1"><strong>Paid Amount : 
						<?php 
                    ...
                    $luggage=mysqli_query($objConn,"select * from power_luggage where  ((luggage_from = ".$routewise3[0]." and luggage_paymethod = 'Paid') or (luggage_to = ".$routewise3[0]." and luggage_paymethod = 'To Pay')) and create_date>='".date('Y-m-d')."' and create_date<='".date('Y-m-d')."'") or die("Vehicle error:".mysql_error());
					$luggage_num=mysqli_num_rows($luggage);
					if($luggage_num==0)
					{
                    ?>
