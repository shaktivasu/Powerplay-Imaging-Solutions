...
{
	//echo"select * from shakthi_collections where route_id= 21  and  collection_date>='".$_REQUEST['frmdate']."' and collection_date<='".$_REQUEST['todate']."'    and collection_flag<>2   order by collection_date";
	$time = strtotime($frmdate.' -1 day');
	$time1 = strtotime($todate.' -1 day');
    					 $fromdatesr = date("Y-m-d", $time);
						  $todatesr = date("Y-m-d", $time1);
	// $news_events_flow=mysqli_query($objConn,"select * from power_luggage where  (((luggage_from = 21 or luggage_from in ( select id from power_route where route_group = 'Coimbatore') )  and luggage_paymethod = 'Paid' and (create_date between  '".$frmdate."' and '".$todate."')  ) or ( ((luggage_to = 21 or luggage_to in ( select id from power_route where route_group = 'Coimbatore') ) )  and luggage_paymethod = 'To Pay'  and (create_date between  '".$fromdatesr."' and '".$todatesr."') ) or ((luggage_from = 21 or luggage_from in ( select id from power_route where route_group = 'Coimbatore') )  and luggage_paymethod = 'Expenses'   and  (create_date between  '".$frmdate."' and '".$todate."')))   and luggage_status != 'Cancelled' and create_date > '2019-03-30'     order by create_date  ") or die("Collection error:".mysql_error());
	
	
	
	$news_events_flow=mysqli_query($objConn,"select * from power_luggage where luggage_paymethod = 'Expenses' and create_date between  '".$frmdate."' and '".$todate."' and luggage_status != 'Cancelled'  and create_date > '2019-03-31'");
...
else
{
	...
	$time1 = strtotime($todate.' -1 day');
    					 $fromdatesr = date("Y-m-d", $time);
						  $todatesr = date("Y-m-d", $time1);
 	 $news_events_flow=mysqli_query($objConn,"select * from power_luggage where   luggage_paymethod = 'Expenses' and  create_date between  '".$frmdate."' and '".$todate."' and luggage_status != 'Cancelled' and create_date > '2019-03-31'     order by create_date  ") or die("Collection error:".mysql_error());
	
}

if($_REQUEST['del']=='expenses')
{
	 $news_events_flow=mysqli_query($objConn,"delete from power_luggage where  id =   ".$_REQUEST['id']) or die("Collection error:".mysql_error());
	  header("location:daysheet.php?frmdate=".$_REQUEST['frmdate']."&todate=".$_REQUEST['todate']);
}
if($_REQUEST['del']=='credit')
{
	$news_events_flow=mysqli_query($objConn,"delete from power_collections_credit where  id =   ".$_REQUEST['id']) or die("Collection error:".mysql_error());
	 header("location:daysheet.php?frmdate=".$_REQUEST['frmdate']."&todate=".$_REQUEST['todate']);
}
if($_REQUEST['del']=='agent')
{
	$news_events_flow=mysqli_query($objConn,"delete from power_collections where  id =   ".$_REQUEST['id']) or die("Collection error:".mysql_error());
	 header("location:daysheet.php?frmdate=".$_REQUEST['frmdate']."&todate=".$_REQUEST['todate']);
}
if($_REQUEST['del']=='transfer')
{
	$news_events_flow=mysqli_query($objConn,"delete from power_transfer where  id =   ".$_REQUEST['id']) or die("Collection error:".mysql_error());
	 header("location:daysheet.php?frmdate=".$_REQUEST['frmdate']."&todate=".$_REQUEST['todate']);
}
?><!DOCTYPE HTML>
<html>
    ...
    <table width="100%" border="1" cellspacing="0" cellpadding="0" style="border-collapse:collapse;" class="arrow_box_content top_arrow">
      ...
      <tr>
        ...
        <td width="87" align="center" class="label_txt">&nbsp;</td>
				...
				{
					//echo"update power_luggage set luggage_fees = ".$_POST[$_POST['del'][$i]]."  where id ='".$_POST['del'][$i]."'";					
						mysqli_query($objConn,"update power_luggage set luggage_lrno='".$_POST['lr_'.$_POST['del'][$i]]."',luggage_fees = ".$_POST[$_POST['del'][$i]]."  where id ='".$_POST['del'][$i]."'") or die(mysql_error());
			   ...
			   header("location:daysheet.php?frmdate=".$_REQUEST['frmdate']."&todate=".$_REQUEST['todate']);
				   ...
				   {
					   for($i=0;$i<count($_POST['del1']);$i++)
				{
					//echo"update power_luggage set luggage_fees = ".$_POST[$_POST['del'][$i]]."  where id ='".$_POST['del'][$i]."'";					
						mysqli_query($objConn,"update power_collections_credit set collection_amount = ".$_POST[$_POST['del1'][$i]]."  where id ='".$_POST['del1'][$i]."'") or die(mysql_error());
			   ...
			   header("location:daysheet.php?frmdate=".$_REQUEST['frmdate']."&todate=".$_REQUEST['todate']);
				   ...
				   {
					   for($i=0;$i<count($_POST['del2']);$i++)
				{
					//echo"update power_luggage set luggage_fees = ".$_POST[$_POST['del'][$i]]."  where id ='".$_POST['del'][$i]."'";					
						mysqli_query($objConn,"update power_collections set collection_amount = ".$_POST[$_POST['del2'][$i]]."  where id ='".$_POST['del2'][$i]."'") or die(mysql_error());
			   ...
			   header("location:daysheet.php?frmdate=".$_REQUEST['frmdate']."&todate=".$_REQUEST['todate']);
				   ...
				   {
					   for($i=0;$i<count($_POST['del3']);$i++)
				{
					//echo"update power_luggage set luggage_fees = ".$_POST[$_POST['del'][$i]]."  where id ='".$_POST['del'][$i]."'";					
						mysqli_query($objConn,"update power_transfer set collection_amount = ".$_POST[$_POST['del3'][$i]]."  where id ='".$_POST['del3'][$i]."'") or die(mysql_error());
			   ...
			   header("location:daysheet.php?frmdate=".$_REQUEST['frmdate']."&todate=".$_REQUEST['todate']);
				   ...
				   ?>     
                       ...
                       <div align="right"><input type="submit" value="UPDATE EXPENSES" name="update1" class="btn_small btn_blue" style="height:30px;"></div>
    
    
						<table class="display">
						<thead>
                        <tr>
                        <td colspan="8" align="left" valign="middle" bgcolor="#F9C6F1"><h2>Expenses</h2></td>
                        </tr>
						<tr>
							<th>Sl No</th>
                            <th>Create Date</th>
				   ...
				   ?>
						...
						<tr <?php if($css_set==0) echo "class=gradeX"; else  echo "class=gradeC"; ?> >
                            ...
                            <td class="center"><?php echo date('d-M-Y',strtotime($news_events_flow_row['create_date']));?></td>
                                                        ...
                                                        <input type="text" name="lr_<?php echo $news_events_flow_row['id']?>" value="<?php echo $news_events_flow_row['luggage_lrno'];?>" >
                                                        </td>
							<td align="left">
							<?php 
							echo $news_events_flow_row['luggage_paymethod'];
				   ...
				   echo $routename1;}
							...
							<?php /* if($news_events_flow_row['luggage_paymethod']=='Expenses')
							{
							echo number_format($news_events_flow_row['luggage_fees'],2);
							$totaldebit+=$news_events_flow_row['luggage_fees'];}*/
							...
							?>
							 ...
							 <td  align="center" valign="middle"><a href="daysheet.php?del=expenses&id=<?php echo $news_events_flow_row['id'] ?>&frmdate=<?php echo $frmdate?>&todate=<?php echo $frmdate?>" onclick="return confirmSubmit();"><img src="images/delete.png"></a></td>
