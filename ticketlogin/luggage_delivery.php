...
<br>
                    ...
                    <div align="right"><div class="btn_40_blue ucase">
								<a href="javascript:deleteAll();"><span class="icon finished_work_sl"></span><span>Click to Luggage Delivered</span></a>
							</div></div><br>
					<div class="widget_content">
	...
	if($_POST['Submit'] || (isset($_REQUEST['frmdate']) && isset($_REQUEST['todate'])))
{
	...
	?>
						...
						<thead>
						<tr>
						  <th width="4%"><span class="yellow_pro">DELIVERY</span>	<input type="checkbox"  name="chkAll"  id="chkAll" onclick="checkAll();"></th>
							<th width="4%">
								 Sl No
							</th>
					...
					if($_POST['lrno']!='')
					{
					 ...
					 $luggage=mysqli_query($objConn,"select * from power_ticket where $vecstr and luggage_status = 'Inward'") or die("Vehicle error:".mysql_error());
					$luggage_num=mysqli_num_rows($luggage);
					}
					else
					{
					$luggage=mysqli_query($objConn,"select * from power_ticket where inward_date>='".$frmdate."' and inward_date<='".$todate."' $vecstr and luggage_status = 'Inward'") or die("Vehicle error:".mysql_error());
					$luggage_num=mysqli_num_rows($luggage);	
					}
					if($luggage_num==0)
					{
				   ...
				   ?>
						...
						?>
							...
							<td align="center" <?php echo $cls;?>>
                             <?php if($luggage_row['luggage_status']=='Inward'){?>
							<a href="#" onclick="return CenterWindow(500,500,50,'print.php?lr=<?php echo $luggage_row['luggage_lrno']; ?>')"><img src="images/print.png"/></a>
							<?php  }
...
<img src="images/edit.png"/></a></span>
                    ...
                    $luggage=mysqli_query($objConn,"select * from power_ticket where luggage_status ='Inward' and luggage_to = '".$_SESSION['route']."' and inward_date>='".$frmdate."' and inward_date<='".$todate."' ") or die("Vehicle error:".mysql_error());
					$luggage_num=mysqli_num_rows($luggage);
					if($luggage_num==0)
					{
                    ?>
				   ...
				   ?>
						<tr <?php if($css_set==0) echo "class=gradeX"; else  echo "class=gradeC"; ?>>
						  <td align="center"> <?php if($luggage_row['luggage_status']=='Inward') 
						{
							?><input type="checkbox" name="del[]"  id="del[]" value="<?php echo $luggage_row['id']; ?>"  > 
							<?php
						}else
						...
						?>
							...
							echo $luggage_row['luggage_status'];
							?></td>
							<td align="center" class="center"><?php /*
                            <?php if($luggage_row['luggage_status']=='Inward') {
								?>
							<span>
