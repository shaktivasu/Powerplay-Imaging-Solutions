...
$sqlstr = "select * from  power_ticket where id = '".$bill_no."'";
$res_get = mysqli_query($objConn,$sqlstr) or die(mysql_error());
$res = mysqli_fetch_array($res_get); 
if(isset($_POST['signupsubmit']))
{
$upda="update power_ticket set luggage_status='Delivered' where id = '".$bill_no."'";
$upda1= mysqli_query($objConn,$upda);
?>
	<script>window.close();window.opener.location.reload();</script>
	<?php
}
?>
<html><body onLoad="window.print();"><form id="signupform" name="signupform"  autocomplete="off" method="post" action="#" class="form_container left_label" >
<table width="100%" border="0">
  <tr>
    <td colspan="5" align="center"><strong>Power Play Imaging Solutions,</strong> <br>
  ...
  <tr>
    <td colspan="5"><table width="100%" border="0" align="center">
      <tr>
       ...
       <?php $sqlstr1 = "select * from  power_ticket_prd where luggage_lrno = '".$res['luggage_lrno']."'";
...
?>
 ...
 <?php $subtotal += $total;}?>
  ...
  <?php }?>
  <?php if($res['unloading_charges']>0) {?>
  <tr>
    <td>&nbsp;</td>
    <td>&nbsp;</td>
    <td colspan="2" align="right">Unloading Charges</td>
    <td align="right"><?php echo number_format($res['unloading_charges'],2)?></td>
  </tr>
    <?php }?>
  <tr>
    <td>&nbsp;</td>
    ...
    <td align="right"><strong><?php echo $gradtotal = number_format($subtotal+$res['luggage_doorcharges']+$res['unloading_charges'],2);?></strong></td>
  </tr>
  <tr>
    <td colspan="5"><button id="signupsubmit" name="signupsubmit" type="submit" class="btn_small btn_blue"><span>Delivery</span></button></td>
